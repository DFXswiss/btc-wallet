...
if [ -z "${HANDBOOK_USER:-}" ] || [ -z "${HANDBOOK_PASSWORD:-}" ]; then
  echo "handbook (DFX BTC Taro Wallet): HANDBOOK_USER and HANDBOOK_PASSWORD must both be set and non-empty." >&2
  echo "handbook (DFX BTC Taro Wallet): refusing to start without authentication (fail loud)." >&2
  exit 1
fi
