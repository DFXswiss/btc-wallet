...
export const baseUrl = ENV.dfxApiUrl
export const paymentUrl = ENV.dfxPaymentUrl

const DfxBaseUrl = ENV.dfxApiUrl
const AuthUrl = 'auth'
const UserUrl = 'user'
const KycUrl = 'kyc'
...
const fetchFrom = async <T>(
  ...
  return (
    ...
    // await new Promise().then((session) => buildInit(method, session, data, options?.noJson))
      .then(async (init) => await fetch(`${BaseUrl}/${url}`, init))
      .then(async (response) => {
        if (response.ok) {
          return await response.json().then().catch(() => {})
        }
