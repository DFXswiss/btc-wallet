...
describe('unit - handbook content gate', () => {
  ...
  describe('QR allowlist', () => {
    ...
    it('names both allowlisted receive screens', function () {
      ...
      assert.deepStrictEqual(Object.keys(gate.QR_ALLOWLIST).sort(), [
        'screenshots/04-empfangen-senden/01-erhalten.png',
        'screenshots/08-lightning/03-rechnung-erstellen.png',
      ]);
    });

    it('accepts only a bare Lightning address on the Lightning receive screenshot', function () {
      const receive = 'screenshots/04-empfangen-senden/01-erhalten.png';
      const lightningShot = 'screenshots/08-lightning/03-rechnung-erstellen.png';
      const address = 'bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4';
      const published = new Set([receive, lightningShot]);
      const scan = payload =>
        gate.qrProblems(
          new Map([
            [receive, address],
            [lightningShot, payload],
          ]),
          published,
          gate.QR_ALLOWLIST,
        );

      assert.deepStrictEqual(scan('0123456789abcdef@breez.tips'), []);
      assert.deepStrictEqual(scan('0123456789abcdef@lightning.space'), []);
      for (const payload of ['0123456789abcdef@evil.example', 'lightning:0123456789abcdef@breez.tips', address]) {
        const problems = scan(payload);
        assert.strictEqual(problems.length, 1, payload);
        assert.match(problems[0], /does not match what the allowlist permits/);
      }
    });

    it('rejects a bolt11 invoice on the Lightning receive screenshot', function () {
      const receive = 'screenshots/04-empfangen-senden/01-erhalten.png';
  ...
  describe('the executable, driven as a child process', () => {
    ...
    const ADDRESS = 'bitcoin:bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4';
    const LN_ADDRESS = '0123456789abcdef@breez.tips';
    ...
    function fixture({ extraOnDisk = [], text = 'Guthaben senden und empfangen ', qrBody, ocrBody } = {}) {
      ...
      stub(
        ...
        'zbarimg',
        qrBody ||
          `case "$*" in *--version*) echo "zbarimg 0.23";; *01-erhalten.png*) echo '${ADDRESS}';; *03-rechnung-erstellen.png*) echo '${LN_ADDRESS}';; *) exit 4;; esac`,
      );
      stub(
        bin,
        'tesseract',
    ...
    it('exits non-zero when zbarimg fails on an image', function () {
      ...
      const r = runScript(
        fixture({
          qrBody: `case "$*" in *--version*) echo "zbarimg 0.23";; *plain.png*) exit 1;; *01-erhalten.png*) echo '${ADDRESS}';; *03-rechnung-erstellen.png*) echo '${LN_ADDRESS}';; *) exit 4;; esac`,
        }),
      );
      assert.notStrictEqual(r.status, 0, r.stdout);
