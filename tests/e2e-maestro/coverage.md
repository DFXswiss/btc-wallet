...
| P16 DFX buy through Spark payout       | `flows/16-dfx-buy-to-payment.yaml`                 | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_WALLET_ADDRESS` is missing. Buy mask shows the truncated `E2E_SPARK_WALLET_ADDRESS` plus `Spark`, then `Zahlungsinformation` / `IBAN` / `BIC`; `Invalid signature` is absent. The local Spark payout service then sends `E2E_PAYMENT_SAT` (default 10) to that wallet from the backend Spark account; after an app restart without wiping state, Spark detail `WalletLabel` `Lightning (Spark)` contains `WalletBalance` risen by that amount, not the copied pre-payout text | not run on this head                          | Needs the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, `E2E_SPARK_WALLET_ADDRESS`, and the local loopback Spark payout service (`scripts/spark-payout.js`; the service is not part of this repository). Without them the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The payout is simulated: it is not a DFX BuyCrypto and there is no fiat credit. The address is not read from the screen; the mask shows it truncated and the flow checks that truncated form against the env value. Camera QR is not used. The Spark row only showed the new balance after that restart, not on the still-open screen. |
| P17 DFX sell through Spark send        | `flows/17-dfx-sell-to-payment.yaml`                | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_DEPOSIT_ADDRESS` is missing. If the visible Spark balance is below `E2E_PAYMENT_SAT` (default 10), the counterpart credits that amount and SuccessView shows it / `^ sats$` / `^Maestro-E2E$`; otherwise the existing balance is kept. After opening Spark detail, `WalletLabel`/`WalletBalance` is copied as the pre-send amount; sell `Zahlungsinformation` shows `Adresse`, `Blockchain`, `Spark`, and the truncated on-screen address matching `E2E_SPARK_DEPOSIT_ADDRESS`. That address is then paid through the wallet send path; after a restart Spark `WalletBalance` is no longer that pre-send amount; `onFlowComplete` pays any remaining visible Spark balance minus 4 sat back, or skips without failing when nothing remains above the reserve; counterpart `paid=true` | not run on this head                          | Needs the treasury counterpart, the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, and `E2E_SPARK_DEPOSIT_ADDRESS`. Without them the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The deposit address is not read from the screen; the mask shows it truncated and the flow checks that truncated form against the env value. A bank payout, a real CHF transfer, and a camera QR read stay outside the suite. P17 returns only the Spark remainder after the send; the sent amount does not come back. An abort before the hook can still leave credit on the same identity; the next run re-imports it. The Spark row only showed a newly credited balance after a restart without wiping state. |
...
paths; they have not been measured on this head. P16 asserts the buy mask (truncated
`E2E_SPARK_WALLET_ADDRESS` plus `Spark`, then `IBAN` and `BIC`) and then has the local
Spark payout service send `E2E_PAYMENT_SAT` to that wallet from the backend Spark account.
That payout is simulated: it is not a DFX BuyCrypto, and there is no fiat credit. P17 fills
the Spark wallet from the counterpart if the visible balance is below `E2E_PAYMENT_SAT`,
opens sell, asserts the truncated on-screen address matches `E2E_SPARK_DEPOSIT_ADDRESS`,
pays that address through the wallet send path, and asserts `WalletBalance` has fallen.
Both fail closed rather than skip.
