...
| P16 DFX buy through Lightning payment  | `flows/16-dfx-buy-to-payment.yaml`                 | Starts without wiping state; fails with an explicit assertion if no Spark wallet is already on the device. DFX buy reaches `Zahlungsinformation`, then the Spark row is a non-zero sats balance | not run on this head                          | Needs an existing Spark wallet on the device, a tradable backend account with a free Lightning deposit, the local DFX stack, and a completed buy (fiat credit). That state is set outside the suite; without it the flow fails rather than skip. |
| P17 DFX sell through Lightning payment | `flows/17-dfx-sell-to-payment.yaml`                | Starts without wiping state; fails with an explicit assertion if no Spark wallet is already on the device. If the visible Spark balance is below `E2E_PAYMENT_SAT` (default 10), the counterpart credits that amount and SuccessView shows it / `^ sats$` / `^Maestro-E2E$`; otherwise the existing balance is kept. After opening Spark detail, `WalletLabel`/`WalletBalance` is copied as the pre-sell amount; sell IBAN form then native `Verkauf bestätigen` is paid; after a restart Spark `WalletBalance` is no longer that pre-sell amount; `onFlowComplete` pays any remaining visible Spark balance minus 4 sat back, or skips without failing when nothing remains above the reserve; counterpart `paid=true` | not run on this head                          | Needs an existing Spark wallet on the device, a tradable backend account with a free Lightning deposit, the local DFX stack, and the DFX sell redirect. That state is set outside the suite; without it the flow fails rather than skip. Web clicks after IBAN are unmeasured. A full-amount sell can leave too little to return; the hook then skips. An abort before the hook can still leave credit on the kept wallet. The Spark row only showed a newly credited balance after a restart without wiping state. |
...
verified status, a non-zero limit, and Lightning deposit addresses. P16 and P17 also need an
existing Spark wallet on the device and a free Lightning deposit; they do not create a wallet.
That state is set outside the flow, and each extended section says so in a comment. Without it
those flows fail, which is the honest behaviour: they do not paper over a stack that cannot
serve them. They fail rather than skip.
