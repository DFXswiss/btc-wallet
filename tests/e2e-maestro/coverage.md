...
| P2 Create Spark wallet                 | `flows/02-create-spark-wallet.yaml`                | Full wallet row `Bitcoin, Lightning (Spark), 0 sats` visible; `Lightning-Wallet` add row not visible                                                                                    | not re-run on this assertion                  | Ends after the Spark creation.                                                                        |
| P3 Spark receive state        | `flows/03-spark-lightning-address.yaml`            | Spark receive shows a `spark1` address; no reusable Lightning address or BOLT11 invoice is asserted                                                                         | not re-run on this assertion                  | It does not register `@lightning.space` or prove Lightning-address receive.                            |
| P4 Spark wallet details from settings  | `flows/04-spark-wallet-details.yaml`               | `Typ` and `Lightning (Spark)` visible                                                                                                                                            | **green**, P01-P07 batch                     | Ends in the wallet detail screen.                                                                     |
...
| P8 Spark receive                         | `flows/08-send-bolt11-to-confirmation.yaml`        | Current file opens Spark receive and asserts a `spark1` address                                                                                                             | not re-run on this assertion                  | Despite its name, it does not enter or pay a BOLT11 invoice.                                          |
| P9 Spark receive                           | `flows/09-send-lnurl-pay.yaml`                     | Current file opens Spark receive and asserts a `spark1` address                                                                                                             | not re-run on this assertion                  | Despite its name, it does not submit an LNURL payment.                                                |
...
| P13 Lightning entry in settings        | `flows/13-settings-lightning-entry.yaml`           | Target screen shows `Typ` and `Lightning (Spark)`                                                                                                                                | **green**, P10/P12/P13 batch                  | Ends in the assigned wallet detail screen.                                                            |
