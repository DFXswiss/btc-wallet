...
| P14 Receive Lightning until balance    | `flows/14-receive-lightning-balance.yaml`          | Treasury pays a Spark invoice for `E2E_PAYMENT_SAT` (default 10); SuccessView shows that amount / `^ sats$` / `^Maestro-E2E$`; after an app restart without wiping state, Spark detail `WalletLabel` `Lightning (Spark)` contains `WalletBalance` matching that amount, not `0 sats`; `onFlowComplete` pays the visible balance minus 4 sat back (6 sat at the default) through the wallet Spark send path to `E2E_SPARK_RETURN_ADDRESS`, or skips without failing if that address is unset | not run on this head                          | Needs the treasury counterpart for the incoming payment (`E2E_TREASURY_*`). The return hook does not use the counterpart; it needs `E2E_SPARK_RETURN_ADDRESS` or it skips. Camera QR is not used. The Spark row only showed the new balance after that restart, not on the still-open screen. The return hook also runs after a failed assertion. An abort before the hook can still leave the credit in a discarded wallet. |
...
| P16 DFX buy through backend chain      | `flows/16-dfx-buy-to-payment.yaml`                 | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_WALLET_ADDRESS` is missing. Buy mask shows the truncated `E2E_SPARK_WALLET_ADDRESS` plus `Spark`, then `Zahlungsinformation` / `IBAN` / `BIC`; `Invalid signature` is absent. The flow snapshots `/v1/transaction`, triggers `PUT /v1/buy/<id>/simulatePayment` for `E2E_BUY_CHF` (default 0.20), waits for a Buy in Completed with id greater than that snapshot, and after an app restart without wiping state asserts Spark detail `WalletLabel` `Lightning (Spark)` contains `WalletBalance` whose sat amount is greater than the pre-buy amount copied from the Spark list row; fails closed if that pre-buy amount did not arrive or did not parse as sats; not a fixed sat credit | **green**, 2026-09-18: `16-dfx-buy-to-payment.yaml` exit 0, 444 s, `passed`. Counter-check on the same head: with `STATE_KIND` flipped to `sell-complete` the flow turns red (exit 1, 399 s), and it passes again (exit 0, 454 s) once the flip is reverted. | Needs the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, `E2E_SPARK_WALLET_ADDRESS`, `E2E_API_URL`, and `E2E_DFX_JWT`. Without them the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The incoming payment is raised through `scripts/dfx-simulate-payment.js`, not a transfer from a real bank. The address is not read from the screen; the mask shows it truncated and the flow checks that truncated form against the env value. Camera QR is not used. The Spark row only showed the new balance after that restart, not on the still-open screen. The sat credit is not asserted as a fixed amount because it follows the rate. Needs `BREEZ_API_KEY` in the build, otherwise `createSparkWallet` fails with `Lightning konnte nicht gestartet werden.` and no flow gets past the import. The closing check is the backend state, a numeric comparison that the post-buy sat amount is greater than the pre-buy amount parsed from the Spark list row, and that `WalletBalance` no longer matches that pre-buy amount. The flow fails closed if the pre-buy copy is missing, empty, or does not parse as sats. |
| P17 DFX sell through backend chain     | `flows/17-dfx-sell-to-payment.yaml`                | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_DEPOSIT_ADDRESS` is missing. No Lightning funding: P16 has to run first so the same identity already holds the Spark payout; the visible Spark balance must be at least `E2E_PAYMENT_SAT` (default 10) or the flow fails rather than skip. After opening Spark detail, `WalletLabel`/`WalletBalance` is copied as the pre-send amount. The flow snapshots `/v1/transaction` before the wallet send, pays `E2E_SPARK_DEPOSIT_ADDRESS` through the wallet send path, waits for a Sell to be booked and then Completed with id greater than that snapshot, then after a restart asserts Spark `WalletBalance` is no longer that pre-send amount; `onFlowComplete` pays any remaining visible Spark balance minus 4 sat through the wallet Spark send path to `E2E_SPARK_RETURN_ADDRESS`, or skips without failing when the address is unset or nothing remains above the reserve | **green**, 2026-09-18: `17-dfx-sell-to-payment.yaml` exit 0, 561 s, `passed`. Counter-check on the same head: with `STATE_KIND` flipped to `buy-complete` the flow turns red (exit 1, 492 s), and it passes again (exit 0, 538 s) once the flip is reverted. Getting there was a clock problem, not a chain problem: the payment now runs before the sell mask because it is what starts the backend chain, the three wait lanes were merged into one, and a block only spends its pause while the goal is unmet. | Needs a prior P16 run on the same identity, the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, `E2E_SPARK_DEPOSIT_ADDRESS`, `E2E_API_URL`, and `E2E_DFX_JWT`. Does not need `E2E_TREASURY_*`. Without the required names the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The deposit address is taken from the env value and paid before the wait; it is not read back from the sell mask. The flows do not open a bank app, do not inspect an IBAN credit, and do not read a camera QR. P17 returns only the Spark remainder after the send, via `E2E_SPARK_RETURN_ADDRESS`; the sent amount does not come back. If that address is unset the hook skips. An abort before the hook can still leave credit on the same identity; the next run re-imports it. The Spark row only showed a newly credited balance after a restart without wiping state. |
...
and waits for the backend to book the pay-in (`sell-booked`) and to complete the sell
(`sell-complete`), each newer than a snapshot taken before the send. A fallen `WalletBalance`
alone is no longer accepted as proof. Leftover Spark is returned through the same send path to
`E2E_SPARK_RETURN_ADDRESS`; if that address is unset the hook skips.

Both fail closed rather than skip. On 2026-09-18 P16 passed on this head against the local
stack (exit 0, 444 s), and a counter-check confirms the green means something: flipping its
target state to `sell-complete` turns it red, reverting makes it pass again. P17 passes as well, with the same kind of counter-check.
Its sell completes only because a loopback service supplies the bank return a local stack cannot
produce; the service fills what a bank would decide and leaves the backend to create the payout
position, find and attach the bank line, and complete the sell.
