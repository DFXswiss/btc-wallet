...
P11/P12 stay red, because the same login answers `400 Invalid signature` there.
...
| P11 DFX buy screen                     | `flows/11-dfx-buy-transition.yaml`                 | Exact page title, `Kaufen` and `Formular` visible; login and `Invalid signature` not visible                                                                                                                                             | **green**, 33 s | Evidences the buy screen reached in the local stack. Entry and buy completion stay unchecked. Against production the login step respectively `400 Invalid signature` remains.            |
| P12 DFX sell screen                    | `flows/12-dfx-sell-transition.yaml`                | Exact page title and `Deine IBAN hinzufügen oder auswählen` visible; login and `Invalid signature` not visible                                                                                                                           | **green**, 29 s | Evidences the sell screen reached in the local stack. IBAN selection and sell completion stay unchecked. Against production the login step respectively `400 Invalid signature` remains. |
...
- With the Spark wallet active and an unpatched API, BUY ends in
...
- Against production the page stops at `Login bei DFX Services` instead; the
  forms are not reached there. The Spark login keeps ending in
  `400 Invalid signature` as long as the API-side change is not deployed.
...
P11 and P12 evidence the screens being reached. On the reviewer's request both
paths were driven further against a local stack. Two limits showed up, and both
are worth stating because they stop short of a completed trade.

**Buy — the API creates the order, the payout is out of reach.** The buy form is
reached, the rate resolves (76 565.19 EUR/BTC for 300 EUR → 0.0039182 BTC), and
the API stores an order whose target address is the Lightning address of the
Spark wallet, together with payment instructions and a bank usage reference.
Completing the buy needs an incoming fiat transfer, which this stack does not
model.

**Sell — reaches the invoice request, then hits the harness.** The sell form is
reached, a test IBAN is accepted, and the API assigns a Lightning deposit route.
The following request fails, for two separate reasons:

```
Exception during PUT '/v1/sell/paymentInfos': TypeError: Cannot read properties of undefined (reading 'includes')
    at HttpService.shouldMock
    at HttpService.get
    at LightningService.getInvoiceByLnurlp
```

First, the local stack has no LNURL-pay service that could act as deposit target
and issue the invoice a sell needs — it runs without outbound network on
purpose. Second, `getInvoiceByLnurlp` passes an undefined URL into
`HttpService.get`, and `shouldMock` calls `url.includes(...)` on it without a
guard, so the request dies with a `TypeError` instead of reporting a deposit
target that yields no invoice. The second point is a defect on the API side and
is reported there.

Reaching even that point required data the stack does not carry: valid price
timestamps, a verified KYC level, a non-zero deposit limit, complete user data
and Lightning deposit addresses. Those were set **in the test database** — never
in application code, and none of it is part of this pull request. That
distinction matters for what the exercise proves: it shows the app drives both
paths correctly and the API accepts what it sends, not that a trade completes
under production conditions. Closing that gap needs an addition to the e2e stack
plus the missing guard on the API side, both outside this repository.

The measurements in this section were taken against an earlier revision of the
API-side change that has since been superseded; they are reported as context for
the reviewer's question, not as evidence for the current stand. The 13-flow
series above is the measurement that carries this branch.
