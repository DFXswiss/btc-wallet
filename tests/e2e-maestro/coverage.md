...
| P16 DFX buy through backend chain      | `flows/16-dfx-buy-to-payment.yaml`                 | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_WALLET_ADDRESS` is missing. Buy mask shows the truncated `E2E_SPARK_WALLET_ADDRESS` plus `Spark`, then `Zahlungsinformation` / `IBAN` / `BIC`; `Invalid signature` is absent. The flow snapshots `/v1/transaction`, triggers `PUT /v1/buy/<id>/simulatePayment` for `E2E_BUY_CHF` (default 0.20), waits for a Buy in Completed with id greater than that snapshot, and after an app restart without wiping state asserts Spark detail `WalletLabel` `Lightning (Spark)` contains `WalletBalance` risen from the copied pre-buy text, not a fixed sat credit | **red**, 2026-09-16: `_setup-import.yaml` failed at `Bitcoin, Lightning (Spark), .*sats, .*` after the Lightning-add tap. The captured screen showed `Lightning konnte nicht gestartet werden. (Error)` with `Wiederholen`; two retries still left no Spark row. last-run (third attempt) `16-dfx-buy-to-payment.yaml` exit 1, 159 s, `assertion-failed`. Did not reach the buy mask, `simulatePayment`, or the backend wait. First attempt aborted at YAML parse of unquoted `MIN_TX_ID` ternary (`: `); that line is now quoted. | Needs the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, `E2E_SPARK_WALLET_ADDRESS`, `E2E_API_URL`, and `E2E_DFX_JWT`. Without them the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The incoming payment is raised through `scripts/dfx-simulate-payment.js`, not a transfer from a real bank. The address is not read from the screen; the mask shows it truncated and the flow checks that truncated form against the env value. Camera QR is not used. The Spark row only showed the new balance after that restart, not on the still-open screen. The sat credit is not asserted as a fixed amount because it follows the rate. This run also needed Spark/Breez to accept `createSparkWallet` on the simulator; that start failed, so the backend chain was not exercised. |
| P17 DFX sell through backend chain     | `flows/17-dfx-sell-to-payment.yaml`                | Imports a fixed Spark identity through `_setup-import.yaml` (`E2E_SPARK_MNEMONIC`); fails closed if `E2E_SPARK_DEPOSIT_ADDRESS` is missing. If the visible Spark balance is below `E2E_PAYMENT_SAT` (default 10), the counterpart credits that amount and SuccessView shows it / `^ sats$` / `^Maestro-E2E$`; otherwise the existing balance is kept. After opening Spark detail, `WalletLabel`/`WalletBalance` is copied as the pre-send amount; sell `Zahlungsinformation` shows `Adresse`, `Blockchain`, `Spark`, and the truncated on-screen address matching `E2E_SPARK_DEPOSIT_ADDRESS`. The flow snapshots `/v1/transaction` before the wallet send, pays that address through the wallet send path, after a restart asserts Spark `WalletBalance` is no longer that pre-send amount, then waits for a Sell to be booked and then Completed with id greater than that snapshot; `onFlowComplete` pays any remaining visible Spark balance minus 4 sat back, or skips without failing when nothing remains above the reserve; counterpart `paid=true` | **red**, 2026-09-16: same `_setup-import.yaml` Spark-start failure as P16 (`Lightning konnte nicht gestartet werden. (Error)` after add and two `Wiederholen` taps). last-run `17-dfx-sell-to-payment.yaml` exit 1, 167 s, `assertion-failed`. Did not reach sell, the wallet send, or the backend wait. `onFlowComplete` ran and skipped the return (`refund skipped: not-visible`). `E2E_TREASURY_URL` was unset; that counterpart was not the blocking failure because the hook never had a Spark balance to return. | Needs the treasury counterpart, the local DFX stack, a tradable backend account for that identity (`scripts/seed-local-backend.sql`), `E2E_SPARK_MNEMONIC`, `E2E_SPARK_DEPOSIT_ADDRESS`, `E2E_API_URL`, and `E2E_DFX_JWT`. Without them the flow fails rather than skip. Missing trade approval fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture. The deposit address is not read from the screen; the mask shows it truncated and the flow checks that truncated form against the env value. The flows do not open a bank app, do not inspect an IBAN credit, and do not read a camera QR. P17 returns only the Spark remainder after the send; the sent amount does not come back. An abort before the hook can still leave credit on the same identity; the next run re-imports it. The Spark row only showed a newly credited balance after a restart without wiping state. |
...
only then asserts that the Spark balance has risen. The payout is performed by DFX itself over
its own Spark payout path; no local helper is involved. What stays synthetic is the fiat leg:
the incoming bank payment is raised through the backend's own DEV/LOC simulation route, not by a
real bank transfer.

P17 pays the sell deposit address through the wallet send path and then waits for the backend to
book the pay-in (`sell-booked`) and to complete the sell (`sell-complete`), each newer than a
snapshot taken before the send. A fallen `WalletBalance` alone is no longer accepted as proof.

Both fail closed rather than skip. On 2026-09-16 both were run on this head against the local
stack and both were red in `_setup-import.yaml`: Spark/Breez did not start (`Lightning konnte
nicht gestartet werden. (Error)`), so neither flow reached the backend booking it asserts. The
backend chains themselves were measured separately against that stack; this run did not
repeat them.
...
fails on `NUTZERDATEN EINGEBEN` with a pointer to the fixture, not a skip. Without those
