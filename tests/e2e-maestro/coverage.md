...
only then asserts that the visible Spark sat amount is at least the backend payout of this run
(`outputAmount` as whole sat). Limitation: if the wallet already held at least this payout
before the buy, the condition is also satisfied without the new credit. The payout is performed
by DFX itself over its own Spark payout path; no local helper is involved. What stays synthetic
