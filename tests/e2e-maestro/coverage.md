...
target state to `sell-complete` turns it red, reverting makes it pass again. P17 reaches every step of its chain and each one
passes, but the last two cron ticks - the backend attaching the bank line and completing the
sell - land after Maestro cuts the run at roughly ten minutes. Those two were verified in the
database for the rows this flow created.
