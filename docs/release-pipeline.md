...
- **`android/app/build.gradle`**: `versionName` ← `-PMYAPP_VERSION_NAME`, `versionCode` ← `-PMYAPP_VERSION_CODE` (fed by the generator in CI; `2.0.3` fallback for local builds).
- **iOS**: `CFBundleShortVersionString` + build number set from the generator at build time (phase 2).

### 2. Trigger model (two lanes)
- **`auto-tag.yml`** — push to `develop` → next `vX.Y.Z` tag (PATCH+1, floored to the build.gradle marketing version). Pushed via **`TAG_DEPLOY_KEY`** so the tag actually triggers the release workflow (a `GITHUB_TOKEN` push would not).
...
- [x] Single-source version generator drives both platforms from the Git tag; tagless build hard-fails. *(phase 1)*
- [x] `auto-tag` on `develop` produces internal-release tags; `vX.Y.0` = production candidate. *(phase 1)*
...
- [x] Existing Android attestation/code-transparency preserved. *(unchanged)*
